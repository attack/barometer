require File.expand_path(File.dirname(__FILE__) + '/../spec_helper')

describe Barometer::Converters do
  def clear_converters
    @converters_cache = Barometer::Converters.converters
    Barometer::Converters.converters = {}
  end

  def reset_conveters
    Barometer::Converters.converters = @converters_cache
  end

  describe ".register" do
    before { clear_converters }
    after { reset_conveters }

    context "when a 1:1 converter is registered" do
      it "makes the converter available" do
        zipcode_converter = double(:converter, :from => [:short_zipcode])
        Barometer::Converters.register(:zipcode, zipcode_converter)
        Barometer::Converters.find(:short_zipcode, :zipcode).should == zipcode_converter
      end
    end

    context "when a 2:1 converter is registered" do
      it "makes the converter available to each format" do
        coordinates_converter = double(:converter, :from => [:short_zipcode, :zipcode])
        Barometer::Converters.register(:coordinates, coordinates_converter)
        Barometer::Converters.find(:short_zipcode, :coordinates).should == coordinates_converter
        Barometer::Converters.find(:zipcode, :coordinates).should == coordinates_converter
      end
    end
  end

  describe ".find_all" do
    before { clear_converters }
    after { reset_conveters }

    context "when the conversion can be made directly" do
      it "returns the one converter" do
        coordinates_converter = double(:coordinates_converter, :from => [:short_zipcode])
        Barometer::Converters.register(:coordinates, coordinates_converter)

        converter = Barometer::Converters.find_all(:short_zipcode, :coordinates)
        converter.should == coordinates_converter
      end

      it "respects preference" do
        zipcode_converter = double(:zipcode_converter, :from => [:short_zipcode])
        Barometer::Converters.register(:zipcode, zipcode_converter)

        coordinates_converter = double(:coordinates_converter, :from => [:short_zipcode])
        Barometer::Converters.register(:coordinates, coordinates_converter)

        converter = Barometer::Converters.find_all(:short_zipcode, [:foo, :coordinates, :zipcode])
        converter.should == coordinates_converter
      end
    end

    context "when the conversion can only be made indirecty" do
      it "returns multiple converters" do
        coordinates_converter = double(:coordinates_converter, :from => [:geocode])
        Barometer::Converters.register(:coordinates, coordinates_converter)

        geocode_converter = double(:geocode_converter, :from => [:short_zipcode])
        Barometer::Converters.register(:geocode, geocode_converter)

        converter = Barometer::Converters.find_all(:short_zipcode, :coordinates)
        converter.should == [geocode_converter, coordinates_converter]
      end

      it "respects preference" do
        zipcode_converter = double(:zipcode_converter, :from => [:geocode])
        Barometer::Converters.register(:zipcode, zipcode_converter)

        coordinates_converter = double(:coordinates_converter, :from => [:geocode])
        Barometer::Converters.register(:coordinates, coordinates_converter)

        geocode_converter = double(:geocode_converter, :from => [:short_zipcode])
        Barometer::Converters.register(:geocode, geocode_converter)

        converter = Barometer::Converters.find_all(:short_zipcode, [:foo, :coordinates, :zipcode])
        converter.should == [geocode_converter, coordinates_converter]
      end
    end

    context "when the conversion cannot be made" do
      it "returns nil" do
        converter = Barometer::Converters.find_all(:short_zipcode, :foo)
        converter.should be_nil
      end
    end
  end
end
