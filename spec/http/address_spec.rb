...
describe Barometer::Http::Address do
  describe "#url" do
    it "returns the initialized url" do
      address = Barometer::Http::Address.new("http://www.example.com")
      address.url.should == "http://www.example.com"
    end

    it "ignores query params in the url" do
      address = Barometer::Http::Address.new("http://www.example.com?foo=bar")
      address.url.should == "http://www.example.com"
    end
  ...
  describe "#query" do
    it "returns the initialized query" do
      address = Barometer::Http::Address.new("", {"foo" => "bar"})
    ...
    it "combines the extracted query and the provided query" do
      address = Barometer::Http::Address.new("http://www.example.com?foo=bar", :foz => :baz)
